...
def wait_server_ready(logfile, search_pattern, **retry_kwargs):
    """Wait up to timeout for server log to indicate router has fully functional.
    Takes same keyword arguments as retry to control the timeout"""
    def peek_at_log():
    ...
    try:
        ...
        raise Exception("peek_at_log timeout waiting for server ready: %s" % (e))
...
class Qdrouterd(Process):
    ...
    def __init__(self, name=None, config=Config(), pyinclude=None, wait=True, wait_for_server_log=True,
        ...
        if wait_for_server_log:
            # This section makes sure that the router.wait functions can find the
            # SERVER (notice) logs in a file. It fails the test if the logs are
            # not set up with minimum requirements of log.enable and log.outputFile.
            ...
            if server_log:
                ...
                # DEFAULT log entry approved. Use it for server log scraping.
    ...
    def wait_ready(self, **retry_kwargs):
        """Wait for ports and connectors to be ready or for server log indication"""
